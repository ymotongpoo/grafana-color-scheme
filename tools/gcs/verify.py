...
def check_ansi(p: Palette) -> list[Problem]:
    ...
    for vname, v in p.each():
        ...
        for flavor in p.flavors:
            table = v.ansi(flavor)
            missing = [s for s in ANSI_SLOTS if s not in table]
            if missing:
                problems.append(
                    Problem("ANSI", f"{vname}/{flavor} missing slots {missing}")
                )
                continue
            for slot, hexv in table.items():
                try:
                    ...
                    problems.append(Problem("ANSI", f"{vname}/{flavor} slot {slot}: {exc}"))
            if cons.get("slot0_not_base") and table[0] == base:
                problems.append(
                    Problem(
                        "ANSI",
                        f"{vname}/{flavor} slot 0 equals surfaces.base ({base}); "
            ...
            if floor is not None:
                ...
                if ratio + 1e-9 < float(floor):
                    problems.append(
                        Problem(
                            "ANSI",
                            f"{vname}/{flavor} slot 8 ({table[8]}) on base: "
...
def report(p: Palette) -> tuple[list[Problem], str]:
    ...
    if not problems:
        ...
        for vname, v in p.each():
            ...
            counts.append(
                f"  {vname:5} worst accent contrast {worst:.2f}:1 ({worst_token}), "
