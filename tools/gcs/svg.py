...
def _section(title: str, items: list[tuple[str, str, str]], x0: int, y0: int,
             page_fg: str) -> tuple[list[str], int]:
    body = [
...
def _board(v: Variant) -> str:
    ...
    ansi_dim = [(f"ansi {i}", v.ansi_dim[i], ratio(v.ansi_dim[i])) for i in range(16)]
...
def _ansi_board(v: Variant) -> str:
    """dim next to vivid, so the difference is visible at a glance."""
    base = v.surfaces["base"]
    cols = 8
    width = PAD * 2 + cols * (CELL_W + 8) - 8
    body: list[str] = []
    ...
    body.append(
        f'<text x="{PAD}" y="{PAD + 6}" font-family="{MONO}" font-size="14" '
        f'font-weight="700" fill="{v.text["bright"]}">{_esc(v.label)} ANSI</text>'
    )
    saved_cols = globals()["COLS"]
    globals()["COLS"] = cols
    try:
        for flavor in ("dim", "vivid"):
            table = v.ansi(flavor)
            items = [
                (str(i), table[i], f"{C.contrast(table[i], base):.2f}:1") for i in range(16)
            ]
            chunk, y = _section(f"{flavor} (slots 9-14 differ)", items, PAD, y, v.text["fg"])
            body += chunk
    finally:
        globals()["COLS"] = saved_cols

    height = y + PAD - 12
    return (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" '
        f'viewBox="0 0 {width} {height}" role="img" '
        f'aria-label="{_esc(v.label)} ANSI palette">\n'
        f'<rect width="{width}" height="{height}" fill="{base}"/>\n'
        + "\n".join(body)
        + "\n</svg>\n"
    )
...
def emit(p: Palette) -> dict[str, str]:
    ...
    for _name, v in p.each():
        ...
        files[f"ansi-{v.name}.svg"] = _ansi_board(v)
    return files
